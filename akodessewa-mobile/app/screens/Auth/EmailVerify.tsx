...
type EmailVerifyScreenProps = StackScreenProps<RootStackParamList, 'EmailVerify'>;

const EmailVerify = ({navigation} : EmailVerifyScreenProps) => {
       ...
       <View style={[GlobalStyleSheet.container,{paddingVertical:20}]}>
        ...
        <View style={[GlobalStyleSheet.border,{flex:1,borderColor:colors.border}]}>
            <View style={[GlobalStyleSheet.container,{flexGrow:1,marginTop:5}]}>
                <ScrollView>
                    <Text style={[FONTS.fontMedium,{fontSize:20,color:colors.title,}]}>Forgot Password</Text>
                    <Text style={[FONTS.fontRegular,{fontSize:15,color:colors.text}]}>Enter the email associated with your account and we'll send and email to reset your password</Text>
                    <View style={{ marginBottom: 10,paddingTop:20}}>
                        <Text style={[styles.title1,{color:colors.title}]}>Email Address<Text style={{ color: '#FF0000' }}>*</Text></Text>
                        <Input
